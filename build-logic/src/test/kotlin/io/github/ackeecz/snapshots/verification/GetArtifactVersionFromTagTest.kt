package io.github.ackeecz.snapshots.verification

import io.github.ackeecz.snapshots.testutil.buildProject
import io.github.ackeecz.snapshots.util.ExecuteCommand
import io.github.ackeecz.snapshots.util.ExecuteCommandStub
import io.github.ackeecz.snapshots.util.createErrorExecuteCommandResult
import io.github.ackeecz.snapshots.verification.GetTagTest.Companion.BOM_VERSION_TAG_PREFIX
...
private const val PROJECT_NAME = "annotations"
private const val VERSION_PROPERTY_NAME = "ANNOTATIONS_VERSION"

private val EXPECTED_INITIAL_VERSIONS = listOf("2.0.0", "2.0.0-paparazzialpha02")
...
private const val PROPERTIES_FILE_CONTENT = """
    ...
    POM_SCM_CONNECTION=scm:git:github.com/AckeeCZ/ackee-android-snapshots.git
    ...
    ANNOTATIONS_ARTIFACT_ID=snapshots-annotations
    ANNOTATIONS_POM_NAME=Ackee Snapshots Annotations
    ANNOTATIONS_POM_YEAR=2024
    ANNOTATIONS_POM_DESCRIPTION=Common code used in production source set when annotating previews and in a testing source set.
"""
...
internal class GetArtifactVersionFromTagTest : FunSpec({
    ...
    context("get null artifact version when tag exists, properties are parsed, but parsing of the version fails and current project version is initial one") {
        EXPECTED_INITIAL_VERSIONS.forEach { expectedInitialVersion ->
            test(expectedInitialVersion) {
                val tagResult = TagResult.Tag("${BOM_VERSION_TAG_PREFIX}1.1.0")
                executeCommand.resultStrategy = ExecuteCommandStub.ResultStrategy.OneRepeating(
                    // Return properties from the above tag
                    ExecuteCommand.Result.Success(properties.writeToString())
                )
                val project = buildProject(name = PROJECT_NAME).also { it.version = expectedInitialVersion }
                val underTest = createSut()
    ...
    context("get null artifact version when fallback to first commit hash and project version matches the expected initial version") {
        EXPECTED_INITIAL_VERSIONS.forEach { expectedInitialVersion ->
            test(expectedInitialVersion) {
                ...
                val project = buildProject().also { it.version = expectedInitialVersion }
                val underTest = createSut()
