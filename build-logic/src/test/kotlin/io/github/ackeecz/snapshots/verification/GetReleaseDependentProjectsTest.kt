...
import io.github.ackeecz.snapshots.testutil.IMPLEMENTATION_CONFIGURATION
import io.github.ackeecz.snapshots.testutil.addDependencies
import io.github.ackeecz.snapshots.testutil.addImplementationDependencies
...
internal class GetReleaseDependentProjectsTest : FunSpec({
    ...
    test("exclude specific dependent projects") {
        // Arrange
        val rootProject = buildProject(name = "root")
        val checkedProject = buildProject(name = "checked", parent = rootProject)
        val projectsToBeExcluded = listOf(
            "sample",
        ).map { name ->
            buildProject(name = name, parent = rootProject)
