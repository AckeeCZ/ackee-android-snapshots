...
2. If you updated Paparazzi to a newer unstable version, don't forget to reflect this in the 
   `paparazzi` and `bom` versions. If you updated Paparazzi to the stable version, remove the
   paparazzi suffixes and this step from this releasing notes.
3. Update `CHANGELOG` for the new release.
4. Create a tag for the new BOM version in the required format.
5. Run `prePublishCheck` Gradle task and fix all found issues if needed.
6. If you were forced to do some changes, fast-forward the tag to the latest commit. Don't forget
